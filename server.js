...
app.post('/api/products', async (req, res) => {
    const { codigo, descripcion, cantidad, tipo } = req.body;
    try {
...
app.get('/api/report', async (req, res) => {
    ...
    try {
        ...
        result.rows.forEach(row => {
            doc.fontSize(12).text(`Código Movimiento: ${row.codigo_mov}, Almacén: ${row.cod_almacen}, Producto: ${row.cod_producto}, Fecha: ${row.fecha}, Tipo: ${row.tipo}, Cantidad: ${row.cantidad}`);
        });
